# diagram-tools Specification

## Purpose

Defines the Realtime tools (`set_diagram`, `patch_diagram`) the model uses to author Mermaid diagrams, the uniform tool-result payload contract, the diagram-architect persona system instructions, the model context-injection rules, and the multilingual panic-reset path.
